  ...
  const loadCompleteData = async () => {
    try {
      ...
      const enrichedBatches = (batchesData || []).map((batch: any) => {
        ...
        infertile_eggs: batch.fertility_analysis?.infertile_eggs,
        fertility_percent: batch.fertility_analysis?.fertility_percent,
        hatch_percent: batch.residue_analysis?.hatch_percent,
        hof_percent: batch.residue_analysis?.hof_percent,
        hoi_percent: batch.residue_analysis?.hoi_percent,
        if_dev_percent: batch.residue_analysis?.if_dev_percent,
        analysis_date: batch.fertility_analysis?.analysis_date,
        sample_size: batch.residue_analysis?.sample_size || batch.fertility_analysis?.sample_size || 648,
        technician_name: batch.residue_analysis?.lab_technician || batch.fertility_analysis?.technician_name,
        fertility_notes: batch.fertility_analysis?.notes,
        fertility_id: batch.fertility_analysis?.id,
        // Flatten residue data
        lab_technician: batch.residue_analysis?.lab_technician,
        total_residue_count: batch.residue_analysis?.total_residue_count,
        unhatched_fertile: batch.residue_analysis?.unhatched_fertile,
        pipped_not_hatched: batch.residue_analysis?.pipped_not_hatched,
        malformed_chicks: batch.residue_analysis?.malformed_chicks,
        contaminated_eggs: batch.residue_analysis?.contaminated_eggs,
        pip_number: batch.residue_analysis?.pip_number,
        live_pip_number: batch.residue_analysis?.live_pip_number,
        dead_pip_number: batch.residue_analysis?.dead_pip_number,
        early_dead: batch.residue_analysis?.early_dead,
        mid_dead: batch.residue_analysis?.mid_dead,
        late_dead: batch.residue_analysis?.late_dead,
        residue_percent: batch.residue_analysis?.residue_percent,
        residue_hatch_percent: batch.residue_analysis?.hatch_percent,
        residue_hof_percent: batch.residue_analysis?.hof_percent,
        residue_hoi_percent: batch.residue_analysis?.hoi_percent,
        residue_if_dev_percent: batch.residue_analysis?.if_dev_percent,
        residue_analysis_date: batch.residue_analysis?.analysis_date,
        residue_fertile_eggs: batch.residue_analysis?.fertile_eggs,
        residue_infertile_eggs: batch.residue_analysis?.infertile_eggs,
        ...
        mold: batch.residue_analysis?.mold,
        abnormal: batch.residue_analysis?.abnormal,
        malpositioned: batch.residue_analysis?.malpositioned,
        upside_down: batch.residue_analysis?.upside_down,
        dry_egg: batch.residue_analysis?.dry_egg,
        ...
        temp_front_top_left: batch.qa_monitoring?.[0]?.temp_front_top_left,
        temp_front_top_right: batch.qa_monitoring?.[0]?.temp_front_top_right,
        temp_front_mid_left: batch.qa_monitoring?.[0]?.temp_front_mid_left,
        temp_front_mid_right: batch.qa_monitoring?.[0]?.temp_front_mid_right,
        temp_front_bottom_left: batch.qa_monitoring?.[0]?.temp_front_bottom_left,
        temp_front_bottom_right: batch.qa_monitoring?.[0]?.temp_front_bottom_right,
        temp_middle_top_left: batch.qa_monitoring?.[0]?.temp_middle_top_left,
        temp_middle_top_right: batch.qa_monitoring?.[0]?.temp_middle_top_right,
        temp_middle_mid_left: batch.qa_monitoring?.[0]?.temp_middle_mid_left,
        temp_middle_mid_right: batch.qa_monitoring?.[0]?.temp_middle_mid_right,
        temp_middle_bottom_left: batch.qa_monitoring?.[0]?.temp_middle_bottom_left,
        temp_middle_bottom_right: batch.qa_monitoring?.[0]?.temp_middle_bottom_right,
        temp_back_top_left: batch.qa_monitoring?.[0]?.temp_back_top_left,
        temp_back_top_right: batch.qa_monitoring?.[0]?.temp_back_top_right,
        temp_back_mid_left: batch.qa_monitoring?.[0]?.temp_back_mid_left,
        temp_back_mid_right: batch.qa_monitoring?.[0]?.temp_back_mid_right,
        temp_back_bottom_left: batch.qa_monitoring?.[0]?.temp_back_bottom_left,
        temp_back_bottom_right: batch.qa_monitoring?.[0]?.temp_back_bottom_right,
        temp_avg_overall: batch.qa_monitoring?.[0]?.temp_avg_overall,
        temp_avg_front: batch.qa_monitoring?.[0]?.temp_avg_front,
        temp_avg_middle: batch.qa_monitoring?.[0]?.temp_avg_middle,
        temp_avg_back: batch.qa_monitoring?.[0]?.temp_avg_back,
        };
      });
