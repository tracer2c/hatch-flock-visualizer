...
const MultiSetterQAWorkflow: React.FC = () => {
  ...
  const handleSubmitQA = async (data: {
    ...
    try {
      const result = await submitMachineLevelQA(
        {
          ...
          day_of_incubation: 0, // Will be determined per position
      ...
      toast.success('Machine-level QA entry saved successfully!');
